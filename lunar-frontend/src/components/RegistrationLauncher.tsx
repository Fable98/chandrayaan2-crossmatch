...
function PointOverlay({ points, side, imgWidth, imgHeight }: {
  points: MatchPoint[];
  ...
  return <div className="pointer-events-none absolute inset-0">{points.map((point, index) => {
    ...
    return <span key={`${side}-${index}`} className="absolute h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white shadow-[0_0_0_2px_rgba(15,23,42,.55)]" style={{ left: `${(value[0] / (imgWidth || 512)) * 100}%`, top: `${(value[1] / (imgHeight || 512)) * 100}%`, backgroundColor: pointColor(point.confidence) }} />;
  })}</div>;
}
...
export default function RegistrationLauncher() {
  ...
  const sourcePrimarySrc = result?.source_url
    ...
    : (result?.warped_url ? absoluteUrl(result.warped_url) : sourcePreview);
  const sourceFallbackSrc = result?.warped_url ? absoluteUrl(result.warped_url) : sourcePreview;

  const referencePrimarySrc = result?.reference_url
    ? absoluteUrl(result.reference_url)
    : referencePreview;
