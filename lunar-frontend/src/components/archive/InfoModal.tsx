...
export default function InfoModal({ content, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-[#1f326e] bg-[#091540] text-white shadow-xl">
        {/* Header Bar */}
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
          ...
          <div className="space-y-4 text-xs leading-relaxed text-[#ABD2FA]">
          ...
          {content.specs && (
            <div className="rounded-md border border-[#1f326e] bg-[#12235c] p-4">
              <span className="text-xs font-medium uppercase tracking-wider text-[#ABD2FA] block mb-3">
                Technical Specifications &amp; Details
              </span>
              <div className="space-y-2 text-xs">
                {content.specs.map((s, idx) => (
                  <div key={idx} className="flex justify-between border-b border-[#1f326e] pb-1.5">
                    <span className="text-[#ABD2FA]">{s.label}</span>
        ...
        <div className="flex items-center justify-between border-t border-[#1f326e] bg-[#12235c] px-6 py-3 text-xs">
          <span className="text-[#ABD2FA]">ISRO Chandrayaan-2 Cross-Match</span>
          <button
            onClick={onClose}
