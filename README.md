...
* **Lunar Reference (LRO NAC)**: OHRC↔LRO NAC optical pairs at ~3.6–4.5× native scale ratio. **Evidence tiles are real downloaded CDRs** (`reference_provenance=real_downloaded_cdr`, see `data_preprocessing_pipeline/lro_nac_real/` and the 15-CDR acquisition registry `data_preprocessing_pipeline/lro_cdr_registry.json`); synthetic OHRC-derived proxies were removed from tracking and their numbers are not cited anywhere in this document.
...
4. **Phase Congruency Extraction:** Single-channel 2D Log-Gabor Phase Congruency + normalized NCC/MI. Moderately robust to gain/bias; NOT invariant to diametric shadow reversal (~162° flip yields only fragile LOW fits, never invariance). Multi-channel CFOG tensor is not implemented.
...
Consequently, this pairing does not require the heavy multi-spectral dimensionality reduction required for hyperspectral IIRS. Optical NCC (`multimodal_pair=False`) won only on retired synthetic OHRC-derived proxies (numbers removed from this document). On **real CDRs**, the opposite holds: optical NCC finds **0 candidates** under the true ~104–132° sun gap and 16-bit I/F radiometry, while the multimodal MI path (`multimodal_pair=True`, now the runner default) succeeds at **0.30–1.29px fit** on the official path — 2/3 sub-pixel, fragile (5–6 inliers) and `LOW_CONFIDENCE`.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `region_001` | `ch2_ohr_ncp_20210405t160653` | `M1417670274LC` (real CDR) | Lat: $[-3.37^\circ, -3.25^\circ]$<br>Lon: $[336.48^\circ, 336.59^\circ]$ | 6 / 32 | **0.6333 px** | null (`insufficient_points_for_holdout`) | **TRUE** ($<1\,\text{px}$) | 6.0% | 0.0183 | LOW_CONFIDENCE |
| `region_003` | `ch2_ohr_ncp_20210405t160653` | `M1417670274LC` (real CDR) | Lat: $[-3.04^\circ, -2.91^\circ]$<br>Lon: $[336.48^\circ, 336.59^\circ]$ | 5 / 27 | **1.2860 px** | null (`insufficient_points_for_holdout`) | FALSE ($>1\,\text{px}$) | 5.0% | 0.0135 | LOW_CONFIDENCE |
| `region_006` | `ch2_ohr_ncp_20220914t083537` | `M1413636095LC` (real CDR) | Lat: $[5.15^\circ, 5.35^\circ]$<br>Lon: $[234.40^\circ, 234.53^\circ]$ | 5 / 24 | **0.3014 px** | null (`insufficient_points_for_holdout`) | **TRUE** ($<1\,\text{px}$) | 5.0% | 0.0135 | LOW_CONFIDENCE |

> [!TIP]
> **Sub-Pixel Precision & Rigorous Metric Integrity (real CDRs)**:
> - **In-Sample Fit only**: fit RMSE is 0.30–1.29px (`sub_pixel_accurate=true` in 2/3 regions; 003 exceeds 1px), but with 5–6 inliers / 8-DOF homography the fit is fragile and **held-out validation is not computable** (`insufficient_points_for_holdout` in all 3 regions). Absolute RMSE is 0.33–1.18m. Report fit alongside inlier count and tier, never alone. Runs are seeded (`cv2.setRNGSeed(42)`) so published fits reproduce exactly.
> - **Reference Scene Provenance**: 3 real-CDR regions across 2 distinct orbital scenes (`M1417670274LC`, node D, emi 1.7° for 001/003; `M1413636095LC`, node A, emi 32° for 006). Sun gaps are real (~132°/132°/104° OHRC-vs-NAC, convention-approximate). Residuals <1px: 100% (001), 20% (003), 100% (006); <0.5px: 50% (001), 0% (003), 80% (006). Coverage is 5–6% canonical 10×10 with uniformity ~0.014–0.018 — sparse and `LOW_CONFIDENCE` by design threshold; overlap-matched native aspect (001) beats padded-square.
...
| **Phase Congruency Structural Feature Representation** | ✅ Verified | Passing tests in [`tests/test_ablation_smoke.py`](tests/test_ablation_smoke.py). Proven tolerant to contrast/gain shifts; formerly failed closed on diametric 162° reversal — `triplet_new_2022` re-measured 2026-09-11 as fragile 50/6 @1.21px LOW (see §6.4 update), never invariance. |
| **Pre-Match Spatial Suppression (ANMS / SSC)** | ✅ Verified | Passing tests in [`tests/test_spatial_uniformity.py::test_grid_nms_enforces_spatial_uniformity_on_1000_clustered_points`](tests/test_spatial_uniformity.py) and verified across 8 flight regions in §6 with 10×10 canonical grid density budgeting. |
...
| **AI Match Verifier (Supervised Random Forest Gate)** | ✅ Verified | Production Random Forest in [`ML_model/ai_verifier.py`](ML_model/ai_verifier.py) trained on synthetically generated geometrically supervised correspondences using known projective transformations and reprojection-error thresholds (same-sensor + antisolar cross-sensor domains). Random Forest predicts correspondence quality; weighted RANSAC provides geometric verification. Hard veto remains `experimental_stack` (tests in `tests/test_ai_verifier_honesty.py`). |
...
| **Sub-Pixel Refinement** | **Partial — tracker sub-pixel, scene fit fragile** | Two-stage refinement (Fourier Phase Correlation + Lucas-Kanade, 0.08–0.31 px per-point). Full-scene fit: TMC 0.99–1.83 px (1/7 <1px); LRO real-CDR 0.30–1.29px on 5–6 inliers, 2/3 sub-pixel (LOW, no held-out). `sub_pixel_accurate` flag now computed (`fit<1.0`). |
...
| :--- | :--- | :--- | :--- | :--- |
| **Phase 1** | Adaptive Illumination Normalization | Sun-angle-invariant structural edges (180° flip-tested on real samples); shadows boosted, never masked | Homomorphic log decomposition (bilateral smoothing + illumination subtraction) + morphological gradient (`matcher_cfog.adaptive_illumination_normalization`) | Implemented — opt-in via `experimental_stack=True`; default path unchanged |
| **Phase 2** | Multi-Scale Feature Extraction | Structural representation + coarse-to-fine priors | 3-level PC pyramid (`multi_scale_phase_congruency(scales=3)`, always built): L2 global phase-correlation (shift applied only when overlap-recovery/tiling armed, else compute-only), L1 half-scale matching → `H_l0_prior` prior; ~16–20× handled by common-GSD resampling upstream, pyramid spans residual 4× | Implemented + tested (`test_scale_pyramid.py`, `test_phase2_pyramid.py`); L1→L0 live on default path since `5ab379d`; native tiling (working-GSD ratio ≥10×) not triggered on 512px benchmark triplets (pre-resampled factors ≈1×) — genuine OHRC 0.25–0.32 vs TMC-2 4–5.4 m/px (12–21×) would fire |
| **Phase 3** | Coarse-to-Fine Correspondence | Scale handling | L0 finest-scale SSC matching SEEDED by L1 anchors (×2 coords) with `H_l0_prior`-centered half-windows, full-window fallback when prior absent; `finest_scale_only=True` ablation hook forces prior-free path; native full-res tiling branch for ≥10× working-scale disparity (rejoins shared verifier + Gates 2/3/4; own Gate-1 ≥4 check; relaxed NCC 0.20 acceptance); ECC pyramid only in IIRS registrar (`iirs_multimodal_registrar.align_ecc_pyramid`, `num_levels=3`) | Implemented — cascade improves support on all benchmark pairs (+1–4 inliers); tiling covered by clean + stressed synthetic 20× tests only — NO native-resolution orbital pair exists in-repo (all triplets pre-resampled to 512px; ISSDC sources login-walled), so real-data tiling validation stays OPEN |
| **Phase 4** | AI Match Verification | Supervised ML RANSAC weights | RandomForestClassifier (`ML_model/ai_verifier.py`) trained on Euclidean GT (`scripts/generate_ground_truth_matches.py`) | Trained bundle loads; probabilities weight Phase 7 RANSAC. Hard pre-RANSAC veto stays behind `experimental_stack` (measured to reject true low-MI real pairs). |
...
| **Phase 7** | Robust Geometric Estimation | Final transformation | Weighted RANSAC (`estimate_weighted_homography`); 4-point DLT is scoring-only, consensus re-fit must pass Quality Gate 3 or fall back to standard RANSAC | Implemented — AI `ai_inlier_prob` weights when the verifier is trained; Gate 3 rejects ill-conditioned samples |
...
1. **Phase Congruency:** single-channel 2D Log-Gabor structural edges, moderately robust to gain/bias — NOT invariant to diametric shadow reversal (`triplet_new_2022` ~162° now yields fragile LOW fits, not clean refusal — see §6.4 update).
2. **Supervised ML (RANSAC weights, not a hard veto):** `RandomForestClassifier` in `ai_verifier.py` is trained on Euclidean ground-truth correspondences (same-sensor + antisolar cross-sensor domains). Live matcher records now populate `refinement_dx/dy` and `spatial_quality_score`. Probabilities weight Phase 7 RANSAC; a hard pre-RANSAC filter remains opt-in because it vetoed true low-MI real-CDR matches.
