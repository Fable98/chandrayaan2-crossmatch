...
| **Spatially Distributed Matches** | **Delivered** | **Pre-match Spatial Suppression (ANMS / SSC)** and **Post-match Grid Density Budgeting** in [`ML_model/spatial_suppression.py`](ML_model/spatial_suppression.py). Canonical 10×10 coverage is 6–7% with 6–7 anchor inliers on default classical path. Opt-in **Guided Densification** (`enable_guided_densification=True`) expands density to 14–28 inliers (10–14% coverage); $H$-conditioned points are explicitly tracked with `provenance="guided_refill"`, verified against independent ground truth (<2.0px error, passing [`tests/test_guided_densification.py`](tests/test_guided_densification.py)), and held-out validation evaluates on independent anchor inliers to prevent circularity. |
