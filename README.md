...
| **AI Match Verifier (Supervised Random Forest Gate)** | ✅ Verified | Production Random Forest in [`ML_model/ai_verifier.py`](ML_model/ai_verifier.py) trained on 3,980 Euclidean GT rows (same-sensor + antisolar cross-sensor domains). Live matcher populates `spatial_quality_score`/`refinement_dx/dy`; probabilities weight RANSAC. Hard veto remains `experimental_stack` (tests in `tests/test_ai_verifier_honesty.py`). |
